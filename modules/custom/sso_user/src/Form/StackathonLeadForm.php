 ...
 * Contribute form.
 */
class StackathonLeadForm extends FormBase {
  /**
   * {@inheritdoc}
   */
  ...
  public function getFormId() {
    return 'stackathon_lead_form_id'; 
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $node_id = NULL) {
      ...
      if(!empty($userName)) {
        $form['web_container']['your_name'] = array(
         ...
         // HASH_REQUIRED => TRUE,
          ...
          HASH_ATTRIBUTES => array('class' => array('form-control'), 'id' => 'edit-your_name', 'readonly' => 'readonly'),
        ...
        $form['web_container']['your_name'] = array(
         ...
         // HASH_REQUIRED => TRUE,
          HASH_PLACEHOLDER => t('Full name'),
          HASH_ATTRIBUTES => array('class' => array('form-control'), 'id' => 'edit-your_name'),
          HASH_PREFIX=>'<div class="col-md-12 form-group">',
          HASH_SUFFIX=>'<div class="error-msg your-name-error"></div></div>',
          '#title' => t('Your name'),
          '#maxlength' => 30,
      ...
      if(!empty($EnableEmailOTP) && $EnableEmailOTP == 1){
        if(!empty($userMail)){
          $form['web_container']['email'] = array(
            ...
            HASH_ATTRIBUTES => array('class' => array('form-control'),'id' => 'edit-email--2' ,'readonly' => 'readonly'),
            HASH_PREFIX=>'<div class="col-md-12 form-group">',
        ...
        $form['web_container']['email'] = array(
            ...
            // HASH_REQUIRED => TRUE,
            HASH_PLACEHOLDER => t('Email ID'),
            HASH_ATTRIBUTES => array('class' => array('form-control'),'id' => 'edit-email--2'),
            HASH_PREFIX=>'<div class="col-md-12 form-group">',
            HASH_SUFFIX=>'<div class="error-msg your-email-error"></div></div>',
            '#title' => t('Email'),
            '#maxlength' => 40,
    ...
    if(!empty($userMobileNo) || (!empty($disableMobileOTP) && $disableMobileOTP == 1)){
      if(!empty($user) && !empty($userMobileNo)){
        $form['web_container'][UPDATECODE]['mobileno'] = array(
          ...
          HASH_ATTRIBUTES => array('class' => array('form-control mobile-numbr'), 'id' => 'edit-mobileno', 'readonly' => 'readonly'),
    ...
    $form['web_container']['intrstd_prgrm'] = [
      ...
      HASH_ATTRIBUTES => array('class' => array('form-control'), 'id' => 'edit-intrstd_prgrm'),
      HASH_PREFIX=>'<div class="col-md-12 form-group">',
    ...
    $form['web_container']['submit'] = array(
        ...
        HASH_AJAX => array(
            CALLBACK => [$this,'SubmitAjaxCallBackFunction'],
            EVENT => 'click',
            PROGRESS => array(
                TYPE => 'throbber',
                MESSAGE => 'Please Wait...',
  ...
  public function SubmitAjaxCallBackFunction(array &$form, FormStateInterface $form_state, $form_id) {
    ...
    if(empty($form_state->getValue('your_name'))){
      $flag_submit=0;
      $ajax_response->addCommand(new AfterCommand('#edit-your_name', '<div class="unplanErrorClass">Field can not be empty.</div>'));
      $ajax_response->addCommand(new InvokeCommand('#edit-your_name', 'addClass',['error']));
    }
    else{
      if(!$validate_name){
          $flag_submit=0;
          $ajax_response->addCommand(new AfterCommand('#edit-your_name', '<div class="unplanErrorClass">Invalid user name</div>'));
          $ajax_response->addCommand(new InvokeCommand('#edit-your_name', 'addClass',['error']));
      }
    }
    ...
    if(empty($form_state->getValue('email'))){
      $flag_submit=0;
      $ajax_response->addCommand(new AfterCommand('#edit-email--2', '<div class="unplanErrorClass">Field can not be empty.</div>'));
      ...
      if(!filter_var($form_data[EMAIL_ID], FILTER_VALIDATE_EMAIL)){
        $ajax_response->addCommand(new AfterCommand('#edit-email--2', '<div class="unplanErrorClass">Invalid email address</div>'));
        $ajax_response->addCommand(new InvokeCommand('#edit-email--2', 'addClass',['error']));
        $flag_submit=0;
      }else{
        if($email_verified != 1){
          $ajax_response->addCommand(new AfterCommand('#edit-email--2', '<div class="unplanErrorClass">Please Verify Email Id</div>'));
            $ajax_response->addCommand(new InvokeCommand('#edit-email--2', 'addClass',['error']));
            $flag_submit=0;
        }
      }
        ...
        /**GDPR API callback function  start*/
         ...
         $getEnrollmentData = json_decode(file_get_contents($_ENV['DOMAIN_TRAINING_COM'].'NIITDigitalPlatformAPI/api/digital/enrollment/v1/customer/'.$CUSTOMER_ID.'/program/'.$course_code));
          ...
          if(!empty($getEnrollmentData->Data) && $getEnrollmentData->TotalRecords > 0){
            $stackathonLeadFormBtn = '<h3>Congratulations! </h3><p>You have registered for the '.$btnTextmsg.'. Further details will mailed to you shortly.</p><div class="new_mutlistep_apply_btn"><a href="javascript:void(0);" class="clsOpenMyBatchesLink StackathonLeadFormBtn" user_cid="'.$CUSTOMER_ID.'"><span class="btn btn-primary btnApply">'.$btnText.'</span></a></div>';
          }else{
            $stackathonLeadFormBtn = '<p>Thank you for showing your interest. We are experiencing some difficulties in registering you for the '.$btnTextmsg.'. Please try again or try after some time.</p>';
          }
          ...
          $ajax_response->addCommand(new HtmlCommand('.stackathon-lead-form-id', $msg));
    }   
