...
class MultiStepExampleForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $status_data = NULL, $nid = NULL) {
    ...
    $form['popup-wrapper-messages'] = [
      '#type' => 'container',
      '#attributes' => [
        'id' => 'popup-messages-wrapper',
      ],
    ];
    ...
    $form['popup-wrapper']['right_side']['#prefix']='<div class="col-md-12 lead-popup-right"><div class="body-right leadLightBoxRight">'.$gen_field['header_msg'];
    $form['popup-wrapper']['right_side']['#suffix']='</div></div></div>';
    $form['popup-wrapper']['right_side'] += $gen_field['f_field'];
    

    // Attach buttons.
    $form['popup-wrapper']['actions']['#type'] = 'actions';
    $buttons = $this->step->getButtons();
    foreach ($buttons as $button) {
      /** @var \Drupal\ms_ajax_form_popup\Button\ButtonInterface $button */
      
      $form['popup-wrapper']['actions'][$button->getKey()] = $button->build($gen_field['screen_id']);

      if ($button->ajaxify()) {
        // Add ajax to button.
        $form['popup-wrapper']['actions'][$button->getKey()]['#ajax'] = [
          'callback' => [$this, 'loadStep'],
          'wrapper' => 'form-popup-wrapper',
          'effect' => 'fade',
        ];
      }
      ...
      if ($button->getSubmitHandler() && is_callable($callable)) {
        ...
        $form['popup-wrapper']['actions'][$button->getKey()]['#submit_handler'] = $button->getSubmitHandler();
      }
    }
    return $form;
  ...
  public function loadStep(array &$form, FormStateInterface $form_state) {
    ...
    if (!empty($messages)) {
      ...
      $response->addCommand(new HtmlCommand('#popup-messages-wrapper', $messages));
    }
    else {
      // Remove messages.
      $response->addCommand(new HtmlCommand('#popup-messages-wrapper', ''));
    }

    // Update Form.
    $response->addCommand(new HtmlCommand('#form-popup-wrapper', $form['popup-wrapper']));
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    if($this->stepId == 1 && $current_user < 1){
      ...
      if($check_useremail == 1){
        $form['popup-wrapper']['right_side']['openloginpopup']['#value'] = 1;
        $form_state->setErrorByName('openloginpopup', 'Already Registered');
      }
      else{
        $form['popup-wrapper']['right_side']['openloginpopup']['#value'] = '';
      }
    }
