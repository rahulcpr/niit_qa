  ...
  if(!empty($searchedCenterList)){
      ...
      if(in_array($_searchedCenterList['centerCode'], $centerCode)){
        ...
        foreach($courseBatchDetail as $courseBatch){
          ...
          // Check course center code  
            ...
            $batchTimingList .= '<li id="batchStart-'.$_searchedCenterList['centerCode'].'-'.$courseBatch['batchID'].'" '.$display.' onClick="getFormId('.$courseBatch['batchID'].',\''.$courseBatch['SRC_ICD'].'\', '.$courseBatch['batchFees'].')" class="">
        ...
        $proceedButton = '<a href="javascript:void(0)" id="submit-'.$_searchedCenterList['centerCode'].'" onclick="SubmitPreForm(this, \''.$_searchedCenterList['centerCode'].'\')" class="morebtn">Proceed</a>';
      }
    ...
    foreach($searchedCenterList as $_searchedCenterList){
      ...
      */
      if(!in_array($_searchedCenterList['centerCode'], $centerCode)){
        $nonBatchesCenters = 2;
        $proceedButton = '<a href="'.$enrollNow.'&centerId='.$_searchedCenterList['centerId'].'&cityName='.$_searchedCenterList['centerCity'].'&centerName='.$_searchedCenterList['centerName'].'" id="submit-'.$_searchedCenterList['centerCode'].'" target="_blank" class="morebtnpro">Enquire Now</a>';  
        $includeText="";  
        $noBatchAvailable = '<div class="row course_starting_from">
      ...
      if ($nonBatchesCenters == 2) {
        $nonBatchesCenters = 0;
              echo $responseData = '<div class="col-md-12">
                          <div class="centre_box noBatchAvailable" id="noBatchAvailable">

                            <div class="row">
                            <div class="enrollMsg">Online enrolment not accepted</div>
                              <div class="col-sm-7">
                                <h3 id="centerName-'.$_searchedCenterList['centerCode'].'">'.$_searchedCenterList['centerName'].'</h3>
                                <p class="locate_centre">
                                    <a href="javascript:void(0);" class="toltip_link" ><i><img src="'.$basePath.'/sites/default/files/2018-11/icon-locate.png" alt=""></i>  Locate the Centre</a>
